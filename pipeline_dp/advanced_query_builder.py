...
class CountDistinctSpec(abc.ABC):
    pass


@dataclasses.dataclass
class LaplaceCountDistinctSpec(CountDistinctSpec):
    privacy_unit: Optional[ColumnNames]
    default_values_to_ignore: Optional[ValuesList]
    ...
    class Builder:
        ...
        def build(self):
            return LaplaceCountDistinctSpec(
                privacy_unit=self._privacy_unit,
                default_values_to_ignore=self._default_values_to_ignore,
                contribution_bounding_level=self._contribution_bounding_level,
                budget=self._budget)
...
@dataclasses.dataclass
class GaussianCountDistinctSpec(CountDistinctSpec):
    privacy_unit: Optional[ColumnNames]
    default_values_to_ignore: Optional[ValuesList]
    contribution_bounding_level: Optional[ContributionBoundingLevel]
    ...
    class Builder:
        ...
        def build(self):
            return GaussianCountDistinctSpec(
                privacy_unit=self._privacy_unit,
                default_values_to_ignore=self._default_values_to_ignore,
                contribution_bounding_level=self._contribution_bounding_level,
                budget=self._budget)
...
class SumSpec(abc.ABC):
    pass


@dataclasses.dataclass
class LaplaceSumSpec(SumSpec):
    privacy_unit: Optional[ColumnNames]
    default_values_to_ignore: Optional[ValuesList]
    contribution_bounding_level: Optional[ContributionBoundingLevel]
    min_total_value: Optional[float]
    max_total_value: Optional[float]
    budget: EpsBudget
    ...
    class Builder:
        ...
        def setTotalValueBounds(self, minTotalValue: float,
                                maxTotalValue: float):
            self._min_total_value = minTotalValue
            self._max_total_value = maxTotalValue
            return self
        ...
        def build(self):
            return LaplaceSumSpec(
                privacy_unit=self._privacy_unit,
                default_values_to_ignore=self._default_values_to_ignore,
                contribution_bounding_level=self._contribution_bounding_level,
                min_total_value=self._min_total_value,
                max_total_value=self._max_total_value,
                budget=self._budget)


@dataclasses.dataclass
class GaussianSumSpec(SumSpec):
    privacy_unit: Optional[ColumnNames]
    default_values_to_ignore: Optional[ValuesList]
    contribution_bounding_level: Optional[ContributionBoundingLevel]
    min_total_value: Optional[float]
...
class MeanSpec(abc.ABC):
    pass


@dataclasses.dataclass
class LaplaceMeanSpec(MeanSpec):
    privacy_unit: Optional[ColumnNames]
    default_values_to_ignore: Optional[ValuesList]
    contribution_bounding_level: Optional[ContributionBoundingLevel]
    ...
    class Builder:
        ...
        def setValueBounds(self, minValue: float, maxValue: float):
...
@dataclasses.dataclass
class GaussianMeanSpec(MeanSpec):
    privacy_unit: Optional[ColumnNames]
    default_values_to_ignore: Optional[ValuesList]
    contribution_bounding_level: Optional[ContributionBoundingLevel]
    min_value: Optional[float]
...
class VarianceSpec(abc.ABC):
    pass


@dataclasses.dataclass
class LaplaceVarianceSpec(VarianceSpec):
    privacy_unit: Optional[ColumnNames]
    default_values_to_ignore: Optional[ValuesList]
    contribution_bounding_level: Optional[ContributionBoundingLevel]
    min_value: Optional[float]
...
@dataclasses.dataclass
class GaussianVarianceSpec(VarianceSpec):
    privacy_unit: Optional[ColumnNames]
    default_values_to_ignore: Optional[ValuesList]
    contribution_bounding_level: Optional[ContributionBoundingLevel]
    min_value: Optional[float]
...
class QuantilesSpec(abc.ABC):
    pass


@dataclasses.dataclass
class TreeQuantilesSpec(QuantilesSpec):
    privacy_unit: Optional[ColumnNames]
    contribution_bounding_level: Optional[ContributionBoundingLevel]
    min_value: Optional[float]
    max_value: Optional[float]
    ...
    class Builder:
        ...
        def setPrivacyUnit(self, columnNames: ColumnNames):
            self._privacy_unit = columnNames
            return self
...
class Query:
    ...
    def run(self, test_mode: bool = False):
        ...
        for agg_type, output_name, spec, extra_params in self._aggregations:
            ...
            elif isinstance(spec, TreeQuantilesSpec):
                noise_kind = aggregate_params.NoiseKind.LAPLACE
            ...
            if hasattr(spec, 'min_total_value'):
                min_sum = spec.min_total_value
                max_sum = spec.max_total_value
            ...
            if agg_type == "count":
                ...
                metrics = [aggregate_params.Metrics.VARIANCE]
            elif agg_type == "count_distinct":
                metrics = [aggregate_params.Metrics.PRIVACY_ID_COUNT]
            elif agg_type == "quantiles":
                metrics = [
                    aggregate_params.Metrics.PERCENTILE(rank * 100)
                    for rank in extra_params['ranks']
                ]

            # Map min_privacy_units_per_group to pre_threshold if set in GroupBySpec
            # The aggregations use the result of partition selection, but if we want to enforce it again?
            # Usually pre_threshold in aggregations is for privacy ID count thresholding.
            # The doc says setMinPrivacyUnitsPerGroup is on GroupBySpec.
            # We mapped it for select_partitions.
            # For aggregations, we don't typically set pre_threshold unless specified?
            # But AggregateParams has pre_threshold.
            # If we use public_partitions (result of select_partitions), those are already thresholded.
            # So we don't strictly need it here unless we want post-aggregation thresholding?
            # For now, keep it None for aggregations unless explicitly needed.
            ...
            value_col = None
            if agg_type in ["sum", "mean", "variance", "quantiles"]:
                value_col = extra_params.get('value_column')
            ...
            for output_name, res_dict, agg_type in aggregations_results:
                if key in res_dict:
                    ...
                    if agg_type == "count":
                        ...
                        row[output_name] = val_dict['sum']
                    elif agg_type == "mean":
                        row[output_name] = val_dict['mean']
                    elif agg_type == "variance":
                        row[output_name] = val_dict['variance']
                    elif agg_type == "count_distinct":
                        row[output_name] = val_dict['privacy_id_count']
                    elif agg_type == "quantiles":
                        row[output_name] = val_dict
                else:

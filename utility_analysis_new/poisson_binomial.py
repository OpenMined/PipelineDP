...
def compute_exp_std_skewness(ps: Sequence[float]) -> Tuple[float, float, float]:
    exp = np.sum(ps)
    std = np.sqrt(np.sum([p * (1 - p) for p in ps]))
    skewness = np.sum([p * (1 - p) * (1 - 2 * p) for p in ps]) / std**3
...
def compute_pmf_approximation(mean: float, sigma: float, skewness: float,
    ...
    xs = np.arange(-0.5, n + 1, 1)
    cdf_values = G((xs - mean) / sigma)

...
class DpEngineTest(parameterized.TestCase):
    ...
    def _create_dp_engine_default(
            self,
            accountant: budget_accounting.NaiveBudgetAccountant = None,
            backend: pipeline_backend.PipelineBackend = None,
        ...
        dp_engine._report_generators.append(
            report_generator.ReportGenerator(aggregator_params, "test_method"))
    ...
    def test_aggregate_private_partition_selection_keep_everything(self):
        ...
        # partition keys.
        budget_accountant = budget_accounting.NaiveBudgetAccountant(
            total_epsilon=100000, total_delta=1e-10)
    ...
    def test_contribution_bounds_already_enforced_sensible_result(self):
        ...
        accountant = budget_accounting.NaiveBudgetAccountant(total_epsilon=1000,
                                                             total_delta=0.999)
    ...
    def test_aggregate_public_partitions_add_empty_public_partitions(self):
        ...
        budget_accountant = budget_accounting.NaiveBudgetAccountant(
            total_epsilon=100000, total_delta=1 - 1e-10)
    ...
    def create_dp_engine_default(
        ...
        aggregator_params = pipeline_dp.AggregateParams(
            ...
            max_contributions_per_partition=1)
        dp_engine._report_generators.append(
            report_generator.ReportGenerator(aggregator_params,
                                             method_name="test"))
        dp_engine._add_report_stage("DP Engine Test")
        return dp_engine
    ...
    def run_e2e_private_partition_selection_large_budget(self, col, backend):
        ...
        # partition keys.
        budget_accountant = budget_accounting.NaiveBudgetAccountant(
            total_epsilon=100000, total_delta=0.99, num_aggregations=1)
    ...
    def test_run_e2e_count_public_partition_local(self, pld_accounting):
        Accountant = pipeline_dp.PLDBudgetAccountant if pld_accounting else budget_accounting.NaiveBudgetAccountant
        ...
        input = [1, 2, 3]
        public_partitions = [2]
        explain_computation_report = report_generator.ExplainComputationReport()
    ...
    def test_run_e2e_partition_selection_local(self):
        ...
        output = self.run_e2e_private_partition_selection_large_budget(
            input, pipeline_backend.LocalBackend())
    ...
    def test_annotate_call(self, mock_annotate_fn):
        ...
        budget_accountant = budget_accounting.NaiveBudgetAccountant(
            total_epsilon, total_delta, num_aggregations=3)
    ...
    def test_add_dp_noise_check_correct_adding_mechanism_created(
        ...
        # Arrange
        data = [(0, 0)]
        accountant = budget_accounting.NaiveBudgetAccountant(total_epsilon=0.5,
                                                             total_delta=1e-8)
        engine = pipeline_dp.DPEngine(accountant,
    ...
    def test_run_e2e_add_dp_noise(self, output_noise_stddev: bool):
        ...
        accountant = budget_accounting.NaiveBudgetAccountant(total_epsilon=2,
                                                             total_delta=0)
        engine = pipeline_dp.DPEngine(accountant,
                                      pipeline_backend.LocalBackend())
        explain_computation_report = report_generator.ExplainComputationReport()
    ...
    def test_weighted_gaussian_calculate_weights(self):
        ...
        weights_of_one = engine._weighted_gaussian_calculate_weights(
            one_pid_per_pks, data_extractors, 100)
        weights_of_one = list(weights_of_one)
        self.assertLen(weights_of_one, 5)
        for pk, weight in weights_of_one:
            self.assertIn(pk, set(range(5)))
            self.assertAlmostEqual(weight, 1.0)

        # Duplicate entries are ignored.
        one_pid_per_pks_with_dups = one_pid_per_pks * 2
        weights_of_one_with_dups = engine._weighted_gaussian_calculate_weights(
            one_pid_per_pks_with_dups, data_extractors, 100)
        weights_of_one_with_dups = list(weights_of_one_with_dups)
        self.assertLen(weights_of_one_with_dups, 5)
        for pk, weight in weights_of_one_with_dups:
            self.assertIn(pk, set(range(5)))
            self.assertEqual(weight, 1.0)

        # With four contributions per each privacy id, the weight contribution
    ...
    @parameterized.parameters(
        ...
        # Total weight is 100*1 = 100, which is above the threshold.
        dict(num_pids=100, num_pks=1, expected_num_partitions=1),
        # The same number of privacy ids per partition key as the previous case.
        # However, now each privacy id contributes to 10 partitions. As a
        # result, the total weight is now 100/sqrt(10) ~= 31.6, which is in most
        # cases below the threshold.
        dict(num_pids=100, num_pks=10, expected_num_partitions=0),
        # In this case, we have a similar total weight as the previous case,
        # make up of 32 privacy ids each contributing to 1 partition. The result
        # is similar to the previous case, i.e., beeing below the threshold.
    ...
    def test_select_partitions_weighted_gaussian(
        ...
        # min_success_probability of the experiments result in the expected
        ...
        for _ in range(num_experiments):
            ...
            engine._budget_accountant.compute_budgets()
            if len(list(output)) == expected_num_partitions:
                num_successes += 1
        self.assertGreaterEqual(num_successes / num_experiments,

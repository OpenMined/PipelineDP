...
class DPComputationsTest(unittest.TestCase):
  ...
  def _test_laplace_kolmogorov_smirnov(self, num_trials: int, results, expected_mean:float, expected_beta:float):
    laplace_sample = np.random.laplace(expected_mean, expected_beta, num_trials)
    (statistic, pvalue) = stats.ks_2samp(results, laplace_sample)
    self.assertGreaterEqual(pvalue, 0.001)

  def _test_gaussian_kolmogorov_smirnov(self, num_trials: int, results, expected_mean:float, expected_sigma:float):
    guassian_sample = np.random.normal(expected_mean, expected_sigma, num_trials)
  ...
  def _test_laplace_noise(self, num_trials: int, results,
    ...
    self._test_laplace_kolmogorov_smirnov(num_trials, results, expected_mean, expected_beta)
  ...
  def _test_gaussian_noise(self, num_trials: int, results,
    ...
    self._test_gaussian_kolmogorov_smirnov(num_trials, results, expected_mean, expected_sigma)

...
from pipeline_dp.aggregate_params import AggregateParams, MechanismType, Metrics, PartitionSelectionStrategy
...
class UtilityAnalysisEngine(DPEngine):
    ...
    def _create_contribution_bounder(
        ...
        if self._options.aggregate_params.perform_cross_partition_contribution_bounding:
            return utility_contribution_bounders.L0LinfAnalysisContributionBounder(
                self._options.partitions_sampling_prob)
        return utility_contribution_bounders.LinfAnalysisContributionBounder(
            self._options.partitions_sampling_prob)
    ...
    def _create_compound_combiner(
        ...
        for params, min_max_sum_per_partition in data_structures.get_aggregate_params(
            ...
            if Metrics.SUM in aggregate_params.metrics:
                ...
                for i_column, (min_sum,
                    ...
                    if len(min_max_sum_per_partition) == 1:
                        i_column = None  # 1 column, no need to set index
        ...
        return per_partition_combiners.CompoundCombiner(
            internal_combiners, n_sum_aggregations=n_sum_aggregations)

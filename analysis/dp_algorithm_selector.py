...
class DPStrategy:
    ...
    partition_selection_strategy: Optional[pipeline_dp.PartitionSelectionStrategy]
...
class AvailableMethods:
    ...
    def supported_partition_selection_strategies(self)->List[pipeline_dp.PartitionSelectionStrategy]:
        return [pipeline_dp.PartitionSelectionStrategy.TRUNCATED_GEOMETRIC, pipeline_dp.PartitionSelectionStrategy.LAPLACE_THRESHOLDING, pipeline_dp.PartitionSelectionStrategy.GAUSSIAN_THRESHOLDING]
...
class DPAlgorithmSelector:
    ...
    def __init__(self, epsilon:float, delta:float, metric: pipeline_dp.Metric, is_public_partitions:bool, available_methods:AvailableMethods):
    ...
    def _get_dp_strategy_for_public_partitions(self, l0: int, linf: float) -> DPStrategy:
        gaussian_std = self._get_gaussian_std(self._epsilon, self._delta, l0, linf)
    ...
    def _get_gaussian_std(self, epsilon:float, delta:float,  l0: int, linf: float)-> float:
        ...
        return dp_mechanisms.GaussianMechanism(epsilon, delta, sensitivity=l2_sensitivity).std

    def _get_laplace_std(self, epsilon:float, l0: int, linf: float)-> float:
        l1_sensitivity = l0*linf
        return dp_mechanisms.GaussianMechanism(epsilon, sensitivity=l1_sensitivity).std
